...
class Grant {
	 ...
	 */
	constructor(grantObj, targetType, target) {
		this.grant = grantObj;
	...
	createSubgrantFromMasks(masks) {
		...
		var mask = Grant.combineGrants(maskObjs);
		return new Grant(mask, this._targetType, this._target);
	}
	...
	check(k, prefix) {
		...
		if(typeof k == 'string') {
			if(!this.has(k, prefix)) {
				if(this._targetType) {
					return new XError(XError.ACCESS_DENIED, 'Access denied trying to ' + prefix + k + ' a target of type ' + this._targetType, { grantKey: prefix + k, targetType: this._targetType, target: this._target });
				} else {
			...
			return new XError(XError.INTERNAL_ERROR, 'Supplied invalid key to permission checking function', { key: k });
		}
		return null;
	}
	...
	checkMask(mask, obj) {
		...
		if(!mask) return new XError(XError.ACCESS_DENIED, 'Access denied in ' + (maskPath || 'mask') + ' for objects of type ' + this._targetType, { grantKey: maskPath, targetType: this._targetType, target: this._target });
		if(!obj || typeof obj != 'object') return new XError(XError.INTERNAL_ERROR, 'Tried to do permissions match against non-object');

		var maskedOutFields = objtools.getMaskedOutFields(obj, mask);
		if(maskedOutFields.length) return new XError(XError.ACCESS_DENIED, 'Access denied in ' + (maskPath || 'mask') + ' for objects of type ' + this._targetType + ' to access field ' + maskedOutFields[0], { grantKey: maskPath, targetType: this._targetType, target: this._target });
		return null;
	}
