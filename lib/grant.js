...
Grant.prototype.check = function(k) {
	if(typeof k == 'string') {
		if(!this.has(k)) {
			if(this._targetType) {
				return new ZSError(ZSError.ACCESS_DENIED, 'Access denied trying to ' + k + ' a target of type ' + this._targetType, { grantKey: k, targetType: this._targetType, target: this._target });
			} else {
				return new ZSError(ZSError.ACCESS_DENIED, 'Access denied trying to ' + k);
			}
		}
	} else if(Array.isArray(k)) {
		for(var i = 0; i < k.length; i++) {
			var e = this.check(k[i]);
			if(e) return e;
		}
	} else if(typeof k == 'object' && k) {
		for(var key in k) {
			var e2 = this.check(key);
			if(e2) return e2;
		}
	} else {
