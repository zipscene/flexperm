...
class Grant {
			...
			if (!this.has(k, prefix)) {
				if (this._targetType) {
					return new XError(XError.ACCESS_DENIED,
						'Access denied trying to ' + prefix + k + ' a target of type ' + this._targetType,
						{ grantKey: prefix + k, targetType: this._targetType, target: this._target }
					);
				} else {
	...
	checkMask(mask, obj) {
		...
		if (!mask) {
			return new XError(XError.ACCESS_DENIED,
				'Access denied in ' + (maskPath || 'mask') + ' for objects of type ' + this._targetType,
				{ grantKey: maskPath, targetType: this._targetType, target: this._target }
			);
		}
		...
		if (maskedOutFields.length) {
			return new XError(XError.ACCESS_DENIED,
				'Access denied in ' + (maskPath || 'mask') + ' for objects of type ' +
					this._targetType + ' to access field ' + maskedOutFields[0],
				{ grantKey: maskPath, targetType: this._targetType, target: this._target }
			);
		}
		return null;
