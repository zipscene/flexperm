...
PermissionSet.prototype.getTargetGrant = function(targetType, target) {
	...
	if(!tree) return new Grant(false);
	var grantObjects = getPermissionTreeMatchingGrants(tree, target);
	if(!grantObjects.length) return new Grant(false);
	if(grantObjects.length == 1) return new Grant(grantObjects[0]);
	return new Grant(Grant.combineGrants.apply(null, grantObjects));
};
			...
			for(i = 0; i < permArray.length; i++) {
				...
				if(perm.target && typeof perm.target == 'object') {
					for(field in perm.target) {
						if(typeof perm.target[field] != 'object') {
