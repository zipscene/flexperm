...
class PermissionSet {
	 ...
	 * @throws XError
	 * @param {String} target - The target string.
	 * @param {Object} query - A query object. Must be in CommonQuery form.
	 * @param {Object} [queryOptions]
	 * @param {Array} [queryOptions.fields] - Array of fields the user has requested to be returned.
	 * @param {Array} [queryOptions.sort] - Array of fields the user has requested to sort by.
	 * @param {Array} [queryOptions.limit] - The user-requested limit of objects to be returned.
	...
	checkExecuteQuery(target, query, queryOptions, queryTypeGrantName) {
		...
		accessError = queryGrant.check(cQuery.getQueriedFields(), 'readMask.');
		if (accessError) return accessError;
	...
	buildPermissionTree(permArray, permissionVars) {
		...
		function buildTargetTree(permArray) {
			...
			// Normalize grants to numbers to ensure they are in a consistent format
			...
			function addAllPermissions(permArray) {
				for (let perm of permArray) {
					...
					let grant = grantNumbersToObjects(perm.grant);
